...
namespace Business.Handlers.RemoteOfferHistoryModels.ValidationRules
{
    ...
    public class UpdateRemoteOfferHistoryModelValidator : AbstractValidator<UpdateRemoteOfferHistoryModelCommand>
    {
        public UpdateRemoteOfferHistoryModelValidator()
        {
            RuleFor(x => x.Name).NotEmpty();
            RuleFor(x => x.IsActive).NotEmpty();
            RuleFor(x => x.FirstPrice).NotEmpty();
